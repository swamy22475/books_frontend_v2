...
const mapToFrontend = (b) => ({
    id: b.id,
    name: b.name,
    book_class: b.book_class ? b.book_class.split(', ') : [],
    type: b.book_type,
    qty: b.total_qty,
    sets: b.sets_qty,
...
const mapToBackend = (f) => ({
    name: f.name,
    book_class: f.book_class && f.book_class.length > 0 ? f.book_class.join(', ') : null,
    book_type: f.type,
...
const Inventory = () => {
    ...
    const filtered = books.filter(b => {
        ...
        const matchClass = filterClass === 'All' || (b.book_class && b.book_class.includes(filterClass));
        return (
                            ...
                            {loading ? (
                                ...
                                return (
                                    <tr key={b.id}>
                                        ...
                                        <td style={{ color: 'var(--bs-muted)', maxWidth: 150, overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                            {b.book_class && b.book_class.length > 0 ? b.book_class.join(', ') : '—'}
                                        </td>
                    ...
                    <div className="bs-modal bs-modal-lg" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="bs-modal-body">
                            <div className="bs-form-grid">
                                ...
                                <div className="bs-form-group full-width">
                                    <label className="bs-form-label">Select Classes (Multiple)</label>
                                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '10px', background: '#f8f9fa', padding: '15px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                                        {availableClasses.map(c => (
                                            <label key={c} style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', cursor: 'pointer', color: '#475569' }}>
                                                <input 
                                                    type="checkbox" 
                                                    checked={form.book_class.includes(c)}
                                                    onChange={e => {
                                                        ...
                                                        setForm(prev => ({
                                                            ...
                                                            book_class: checked 
                                                                ? [...prev.book_class, c] 
                                                                : prev.book_class.filter(item => item !== c)
                                                        }));
                                                    }}
                                    ...
                                    <div style={{ marginTop: '5px', fontSize: '12px', color: '#64748b' }}>
                                        Selected: {form.book_class.length > 0 ? form.book_class.join(', ') : 'None'}
                                    </div>
                                </div>

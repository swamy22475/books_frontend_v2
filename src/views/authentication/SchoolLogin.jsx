...
const SchoolLogin = () => {
      ...
      if (savedUser) {
        try {
          ...
          if (parsed.role === 'super_admin') {
            ...
            navigate('/school/book-sales', { replace: true });
          }
        } catch { 
          navigate('/school/book-sales', { replace: true });
        }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // 1. Check if user exists in our "Admin Created" local users
      ...
      if (localUser) {
        // Mock successful login for local user
        const userData = {
          id: localUser.id,
          username: localUser.username,
          fullName: localUser.schoolName,
          role: 'school_admin',
          schoolName: localUser.schoolName,
          tenantId: localUser.tenantId
        };
        
        localStorage.setItem('auth_token', 'local_token_' + localUser.id);
        localStorage.setItem('auth_user', JSON.stringify(userData));
        localStorage.setItem('tenant_id', localUser.tenantId);
        localStorage.setItem('is_local_tenant', 'true');

        window.location.href = '/school/book-sales';
        return;
      }

      // 2. Fallback to existing backend authentication
      ...
      if (userData.is_first_login) {
        window.location.href = userData.role === 'super_admin' ? '/super/update-password' : '/school/update-password';
      } else {
        window.location.href = userData.role === 'super_admin' ? '/super/dashboard' : '/school/dashboard';
      }
